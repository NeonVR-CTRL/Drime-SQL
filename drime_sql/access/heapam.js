/**
 * DRIME.SQL - Heap Access Method (heapam rewrite)
...
export class HeapAccessMethod {
  ...
  async heapInsert(relOid, data, xid) {
    ...
    // Add tuple to page
    const itemIdIndex = pageOps.addItem(tupleData);
    if (itemIdIndex < 0) {
    ...
    if (item) {
      ...
      newData.set(updatedHeaderBytes);
      newData.set(item.data.slice(updatedHeaderBytes.length), updatedHeaderBytes.length);
      
      // Delete and re-add with updated header
      pageOps.deleteItem(itemIdIndex);
