/**
 * DRIME.SQL - Cloudflare Worker Handler
...
export class WorkerHandler {
  constructor() {
    ...
    this.db = new DrimeSQLDatabase(CONFIG.DRIME_BUCKET, CONFIG.DRIME_API_KEY);
  ...
  async fetch(request) {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/health') {
        return new Response(JSON.stringify({ status: 'ok', engine: 'DRIME.SQL' }), {
          headers: { 'Content-Type': 'application/json' }
        });
      }
